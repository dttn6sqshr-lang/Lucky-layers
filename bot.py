...
def create_card(card_type, scratched, rewards):
    img = Image.new("RGBA", (520, 320), (255, 255, 255, 255))
    ...
    # Draw 8 hearts in 2 rows x 4 columns
    for i in range(8):
        row = i // 4
        col = i % 4
        x = padding_x + col * gap_x
        y = padding_y + row * gap_y

        if not scratched[i]:
            # Covered heart with overlay
            draw_heart(draw, x, y, heart_size, (180,180,180))  # gray overlay
            draw.text((x+15, y+15), "?", fill="black", font=font)
        else:
            # Reveal heart with reward
...
class ScratchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if user["sugar"] >= 1000 and "1000_sugar" not in user["badges"]:
            ...
            await interaction.user.send(f"🏅 Congratulations! You unlocked the **1000 sugar** badge!")

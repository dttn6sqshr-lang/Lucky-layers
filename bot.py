...
LOG_CHANNEL_ID = 1475295896118755400  # audit logs
...
def create_card(card_type, scratched, rewards):
    ...
    for i in range(8):
        ...
        # Bow above heart
        draw.ellipse((x + heart_size//2 - bow_size//2, y - bow_size, x + heart_size//2 + bow_size//2, y), fill="pink")

        # Show reward if scratched
        if scratched[i]:
            text = rewards[i]
            if is_rare(text):
                for glow in range(3):
...
class ScratchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Animate reveal: reveal in 3 steps
        for _ in range(3):
        ...
        if user["sugar"] >= 1000 and "1000_sugar" not in user["badges"]:
            ...
            await interaction.user.send(f"🏅 You unlocked the 1000 sugar badge!")
...
class CardTypeDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(content=f"Choose recipient for {card_type}", view=view)

...
def create_card_image(card_type, scratched):
    """Generate fully coquette card with hearts, bows, and smooth gradient."""
    ...
    for y in range(height):
        pos = y / (height-1) * (len(rgb_colors)-1)
        i = int(pos)
        frac = pos - i
        if i >= len(rgb_colors)-1:
            ...
            c = tuple(
                int(rgb_colors[i][j] + (rgb_colors[i+1][j]-rgb_colors[i][j])*frac)
                for j in range(3)
            )
        draw.line([(0,y),(width,y)], fill=c)
    
    # Heart panel coordinates
    heart_coords = [
    ...
    for i, (x, y) in enumerate(heart_coords):
        ...
        fill_color = (255,192,203,200) if scratched[i] else (255,182,193,150)
